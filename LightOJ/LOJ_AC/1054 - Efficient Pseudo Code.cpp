/***************************************************
 * Problem Name : 1054 - Efficient Pseudo Code.cpp
...
int main () {
    ...
    for (int t = 1; t <= tc; t++) {
        ...
        for (int i = 0; i < (int) fact.size(); i++) {
            ll p = fact[i].first;
            ll k = (ll) fact[i].second * m;
            ll tmp = (bigMod (p, k + 1LL) % MOD - 1LL + MOD) % MOD;
            ll sum = (tmp * bigMod (p - 1LL, MOD - 2) ) % MOD;
            res = (res * sum) % MOD;
        }
